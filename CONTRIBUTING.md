...
This repo ships a validator (`scripts/validate.py`) that checks frontmatter, conformance to [`schemas/civic_schema.json`](schemas/civic_schema.json), the `civic/0.4` profile declaration, and that each record's `x-civic.relations` matches its prose link-title edges (reciprocally).
