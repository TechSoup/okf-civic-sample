...
This repo ships a validator (`scripts/validate.py`) that checks frontmatter, conformance to [`schemas/civic_schema.json`](schemas/civic_schema.json), the `civic/0.5` profile declaration, and that each record's `x-civic.relations` matches its prose link-title edges (reciprocally). It also enforces the OKF index-file rule (§6/§11): `index.md` files are listings, not concepts, and carry no frontmatter — except the bundle-root `index.md`, which may, to declare `okf_version`.
